...
//check
 findKing: function findKing(color){
  ...
  check: function check(color,form,to){
    //this.fakemove(form, to);
    var tmp = this.pieces[to]; 
    this.pieces[to] = this.pieces[form]; 
    delete this.pieces[form]; 

    var kingPosition = this.findKing(color);

    for (var p in this.pieces){      
      if(this.pieces[p].color != color){
        var lm = this.pieces[p].getMoves(false);
        for (var m in lm){
          if (lm[m] == kingPosition){
            this.fakemove(to, form); 
            console.log(color,form,to,true);       
            return true;
          }
        }
      }
    ...
    //this.fakemove(to, form);
    this.pieces[form] = this.pieces[to]; 
    delete this.pieces[to]; 
    if(tmp) this.pieces[to] = tmp; 
    console.log(color,form,to,false);       
    return false;
  },

  fakemove :function fakemove(form, to) {
    this.pieces[to] = this.pieces[form]; 
    delete this.pieces[form]; 
  },
// end check
  ...
  getMoves: function(form) {
     if (this.pieces[form] === undefined) {
      ...
      return this.pieces[form].getMoves(true);;
  },
