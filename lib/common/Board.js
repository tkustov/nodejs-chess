...
Board.prototype = {
  ...
  getState: function () {
    ...
    for (piece in this.pieces) {
      var curPiece = {};
      curPiece.name = this.pieces[piece].constructor.name;
...
// vbruger 03.04.2016
// inplement method canMove in Board
  canMove: function (form,to) {
    if (!this.isOnBoard(this.reverseTrans(to))) return false;
    if (this.isOpponentPiece(form,this.reverseTrans(to))) return true;
    if (this.isFreeCell(this.reverseTrans(to))) return true;
    return false;
  }
};
