...
exports.acceptInvitation = function(req, res) {
  ...
  //Send to both platers gameId, and than they start game.
  User.findById({_id:userId}, function(err, user) {
    gameSockets.to(user.userRoom).emit('startGame', {gameId: game._id});
  });

  User.findById({_id:opponentId}, function(err, opponent) {
    gameSockets.to(opponent.userRoom).emit('startGame', {gameId: game._id});
  });
...
exports.sendInvitation = function (req, res) {
  ...
  User.findById({_id:opponentId}, function(err, opponent) {
    gameSockets.to(opponent.userRoom).emit('incomingInv', {userId: userId, userName: username});
  });
